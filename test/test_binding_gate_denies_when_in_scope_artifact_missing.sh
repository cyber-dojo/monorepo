...
# template lists {web,dashboard}; only web attests; dashboard is left MISSING. Under the binding model
# "dashboard failed" and "dashboard never ran" are the same MISSING artifact, so this single test
...
echo "## arrange -- only web attests into the binding trail; dashboard never does"
...
# dashboard deliberately NEVER attested (it failed its own gate, or never ran).
...
echo "  OBSERVED binding is_compliant = ${trail_ic} (web=${web_ic})"
echo "  OBSERVED artifacts_statuses.dashboard present = ${dashboard_present} (status=${dashboard_status})"
...
assert_equals "expected dashboard listed as MISSING (not omitted)" "${dashboard_status}" "MISSING"
